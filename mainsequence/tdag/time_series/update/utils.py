...
class UpdateInterface:
    ...
    def _patch_update(self, hash_id, update_kwargs: dict):
    ...
    def _assert_next_update(self, hash_id: str):

        execution_time = datetime.datetime.utcnow().replace(tzinfo=pytz.utc)
        if self.state_data[hash_id]["last_time_index_value"] is None:
            return True

        next_update = self.state_data[hash_id]["next_update"]
        error_on_last_update = self.state_data[hash_id]["error_on_last_update"]
    ...
    def add_ts_to_queue(self, hash_id_list: list):
        pass

    def _build_new_update_for_hash_id(self, hash_id: str):
        try:
            new_update = dict(update_time_start=datetime.datetime.now(pytz.utc),
                              update_time_end=None,
                              update_completed=False,
                              error_on_update=False,
                              last_time_index_value=self.state_data[hash_id]['last_time_index_value'],
    ...
    def set_start_of_execution_batch(self, hash_id_list: list)->list[StartUpdateDataInfo]:
        new_starts, all_starts = {}, {}

        for hash_id in hash_id_list:
            all_starts[hash_id] = self._build_new_update_for_hash_id(hash_id=hash_id)
        in_update_queue = self.get_ts_in_update_queue(hash_id_list=hash_id_list)
        to_place_in_queue = []
        for hash_id, start_data in all_starts.items():
            must_update = self._assert_next_update(hash_id=hash_id)
            if hash_id in in_update_queue:
                # do not update TS that are in queue already () updated by other task
                must_update = False

            start_data["must_update"] = must_update
            start_data["direct_dependencies_hash_id"] = self.state_data[hash_id]["direct_dependencies_hash_id"]
            new_starts[hash_id] = StartUpdateDataInfo(**start_data)


            if must_update == False:
                start_data['update_completed'] = True
                self.set_end_of_execution(hash_id, error_on_update=False)
            else:
                to_place_in_queue.append(hash_id)


        self.add_ts_to_queue(hash_id_list=to_place_in_queue)

        self.updating_in_tree = list(set(hash_id_list) - set(in_update_queue))

        return new_starts

    def set_start_of_execution(self, hash_id: str)->StartUpdateDataInfo:

        start_update_data = self._build_new_update_for_hash_id(hash_id=hash_id)


        must_update = self._assert_next_update(hash_id=hash_id)
        start_update_data["must_update"] = must_update
        start_update_data["direct_dependencies_hash_id"] = self.state_data[hash_id]["direct_dependencies_hash_id"]


        self._patch_update(hash_id=hash_id, update_kwargs=dict(active_update_status="U"))

        return StartUpdateDataInfo(**start_update_data)

    def set_end_of_execution(self, hash_id: str, error_on_update=False, error_message=""):

        if error_on_update == True:
            self.logger.error(f" UPDATE TRACKER ERROR IN: Setting end of execution {hash_id}  {error_message}")


        active_update_status = "S" if error_on_update == False else "E"
        self._patch_update(hash_id=hash_id, update_kwargs=dict(active_update_status=active_update_status,
                                                               active_update=False,

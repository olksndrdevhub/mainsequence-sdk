...
def build_binance_perpetual_futures_asset(execution_venue: ExecutionVenue,api_key:str,api_secret:str):
    """
    ...
    """
    import ccxt
    if execution_venue.symbol in CONSTANTS.CRYPTO_FUTURES_VENUES:
    ...
    # get account information to create base assets
    exchange_info = exchange.fetch('https://fapi.binance.com/fapi/v1/exchangeInfo')
    asset_list,margin_asset_symbol_list=[],[]
    for asset_config in exchange_info["assets"]:
        margin_asset = dict(symbol=asset_config["asset"],
                          name=asset_config["asset"],
                          asset_type=CONSTANTS.ASSET_TYPE_CRYPTO_SPOT,
                          can_trade=False,  # cant trade spot on crypto
                         margin_available_for_futures=True
                          )
        asset_list.append(margin_asset)
        margin_asset_symbol_list.append(asset_config["asset"])

    r = BinanceAsset.batch_upsert(execution_venue_symbol=execution_venue.symbol,
                                                            asset_type=CONSTANTS.ASSET_TYPE_CRYPTO_SPOT,
                                                            asset_config_list=asset_list, timeout=60 * 15)
    ...
    for asset_config in symbols_list:
        if asset_config['contractType']=="PERPETUAL":

            base_asset=dict(symbol=asset_config["baseAsset"],
                                    name=asset_config["baseAsset"],
                                    asset_type=CONSTANTS.ASSET_TYPE_CRYPTO_SPOT,
                                    can_trade=False, #cant trade spot on crypto
                            margin_available_for_futures=asset_config["baseAsset"] in margin_asset_symbol_list
                                    )

            quote_asset_type=CONSTANTS.ASSET_TYPE_CRYPTO_SPOT if asset_config["quoteAsset"]!="USD" else CONSTANTS.ASSET_TYPE_FIAT_CURRENCY
            quote_asset=dict(symbol=asset_config["quoteAsset"],
                            name=asset_config["quoteAsset"],
                            asset_type=quote_asset_type,
                            can_trade=False,  # cant trade spot on crypto
                             margin_available_for_futures=asset_config["quoteAsset"] in margin_asset_symbol_list
                                    )
            future_asset_list.append({"base_asset":base_asset,"quote_asset":quote_asset,
                                      "future_extra_kwargs":dict(
                                                        maintenance_margin_percent=asset_config['maintMarginPercent'],
                                                        required_margin_percent=asset_config['requiredMarginPercent'],
                                                        price_precision=asset_config['pricePrecision'],
                                                        quantity_precision=asset_config['quantityPrecision'],
                                                        base_asset_precision=asset_config['baseAssetPrecision'],
                                                        quote_asset_precision=asset_config['quotePrecision'],
                                                        )
                                      })



    r = BinanceAssetFutureUSDM.batch_upsert_from_base_quote(execution_venue_symbol=execution_venue.symbol,
                                 asset_type=CONSTANTS.ASSET_TYPE_CRYPTO_USDM,
                                 asset_config_list=future_asset_list, timeout=60 * 15)

...
class ScriptResource(BaseModel):
    """A script to be executed by a job."""
    path: str = Field(..., min_length=1, description="The filesystem path to the script.")
...
class Job(BaseModel):
    """A single, named job with its resources and schedule."""
    ...
    def _validate_resources_not_empty(self) -> "Job":
        if not self.resources:
            raise ValueError("The 'resources' list must contain at least one item.")
        return self
...
class ProjectTemplate(BaseModel):
    ...
    @model_validator(mode="after")
    def _validate_jobs_not_empty(self) -> "Project":
        if not self.jobs:
            raise ValueError("The 'jobs' dictionary must contain at least one job.")
        return self
